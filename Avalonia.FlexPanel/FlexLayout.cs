...
namespace Avalonia.FlexPanel
{
	public class FlexLayout : VirtualizingLayout
	{
		...
		protected override void InitializeForContextCore(VirtualizingLayoutContext context)
		{
			...
			var flexState = state as FlexLayoutState;

			if (flexState == null)
			{
				if (state != null)
					throw new InvalidOperationException($"LayoutState must derive from {nameof(FlexLayoutState)}.");

				flexState = new FlexLayoutState(context);
				context.LayoutState = flexState;
			}

			flexState.InitializeForContext(context);
		}

		/// <inheritdoc />
		protected override void UninitializeForContextCore(VirtualizingLayoutContext context)
		{
			var flexState = context.LayoutState as FlexLayoutState;
			flexState?.UninitializeForContext(context);
		}

		protected override Size MeasureOverride(VirtualizingLayoutContext context, Size constraint)
		{
			var flexState = (FlexLayoutState) context.LayoutState;
			...
			flexState.Clear();
			flexState.OrderItems();

			for (var i = 0; i < context.ItemCount; i++)
			{
				var item = flexState.GetItemAt(i);
		...
		protected override Size ArrangeOverride(VirtualizingLayoutContext context, Size arrangeSize)
		{
			var flexState = (FlexLayoutState) context.LayoutState;
			...
			for (var i = 0; i < context.ItemCount; i++)
			{
				var element = flexState.GetItemAt(i);
			...
			if (firstInLine < context.ItemCount)
				ArrangeLine(flexState,
					new FlexLineInfo
					{
						ItemsU = itemsU,
						...
						LineU = uvFinalSize.U,
						ItemStartIndex = firstInLine,
						ItemEndIndex = context.ItemCount,
						ScaleU = scaleU
					});
		...
		private void ArrangeLine(FlexLayoutState state, FlexLineInfo lineInfo)
		{
			var flexDirection = FlexDirection;
			var flexWrap = FlexWrap;
			...
			if (constraintFreeU > 0)
			{
				...
				{
					var flexGrow = state.GetItemAt(i).FlexGrow;// GetFlexGrow(children[_orderList[i].Index]);
			...
			if (constraintFreeU < 0)
			{
				...
				for (var i = 0; i < itemCount; i++)
				{
					var flexShrink = state.GetItemAt(i).FlexShrink;//GetFlexShrink(children[_orderList[i].Index]);
			...
			{
				var child =  state.GetItemAt(i).Element;//children[_orderList[i].Index];
				...
				var alignSelf = state.GetItemAt(i).AlignSelf;//GetAlignSelf(child);
